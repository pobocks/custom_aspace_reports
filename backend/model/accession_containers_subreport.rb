class AccessionContainersSubreport < AbstractSubreport

  #register_subreport('containers', ['accession'])
  ...
  def query_string
    "select distinct       
      ...
      from  (select id, instance_type_id, is_representative
             from  instance
             where instance.accession_id = #{db.literal(@accession_id)}
             and instance.instance_type_id != #{db.literal(@do_enum)} ) as instances
      left outer join sub_container
        on instances.id = sub_container.instance_id
